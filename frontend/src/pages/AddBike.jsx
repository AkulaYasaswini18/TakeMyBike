  ...
  return (
    <div className="add-bike-container">
      <div className="add-bike-card">
        ...
        <div>
          <label>Security Deposit ($)</label>
          <input type="number" name="securityDeposit" value={form.securityDeposit} onChange={handleChange} />
        </div>

        <div>
          <label>Pickup Area</label>
          <input name="area" value={form.location.area} onChange={handleChange} />
        </div>

        <div>
          <label>Condition (New/Excellent/Good/Fair)</label>
        ...
        <div>
          ...
          {previews.length > 0 && (
            <div style={{ marginTop: '10px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {previews.map((preview, idx) => (
                <img key={idx} src={preview} alt="preview" style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
              ))}
            </div>
          )}
        </div>
      ...
      {msg && <p style={{ marginTop: '20px', padding: '10px', backgroundColor: msg.includes('Error') ? '#ffe6e6' : '#e6ffe6' }}>
        {msg}
      </p>}
    </div>
  )
